...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* parse file name (user) */
  char file_name_sub[100];
  strlcpy(file_name_sub, file_name, strlen(file_name)+1);
  char *parse[100];
  char *token, *ptr_tok;
  int cnt_tok = 0;

  if((token = strtok_r(file_name_sub," ", &ptr_tok))!=NULL){
	parse[cnt_tok++] = token;
    while((token = strtok_r(NULL, " ", &ptr_tok))!=NULL)
      parse[cnt_tok++] = token;
  }
  ...
  if (!setup_stack (esp))
    goto done;
  /* Argument passing (user) */
  //esp is stack pointer of user stack(have value already)
  //we must stack the top of pointer reverse
  char **arg_address;
  int word_align_num=0;
  char *word_align;
  arg_address = (char **)malloc(sizeof(char *) * cnt_tok);
  for(i=cnt_tok-1;i>=0;i--){
	  *esp -= (strlen(parse[i])+1); // 1 take NULL pointer
	  strlcpy(*esp,parse[i],strlen(parse[i])+1);
	  arg_address[i] = *esp;
	  word_align_num += (strlen(parse[i])+1);
  }
  //word align part
  word_align_num = 4 - word_align_num % 4;
  *esp -= word_align_num;
  word_align = (char *)calloc(word_align_num, sizeof(char));
  strlcpy(*esp, word_align, word_align_num);
  //stack address
  *esp -= 4; *(char**)*esp = 0;
  for(i = cnt_tok-1; i>=0; i--){
	  *esp -= 4;
